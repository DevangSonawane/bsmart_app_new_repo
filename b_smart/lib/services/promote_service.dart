    ...
    try {
      ...
      return items.map((raw) {
        ...
        final user = item['users'] is Map
            ...
            : (item['user'] is Map ? Map<String, dynamic>.from(item['user'] as Map) : null);
        ...
        final username =
            (pickedUser?['username'] ?? pickedUser?['full_name'] ?? pickedUser?['name'] ?? 'User').toString();
        ...
        if (mediaList.isNotEmpty) {
          ...
          if (first is String) {
            ...
            videoUrlObj = fm['url'] ?? fm['fileUrl'] ?? fm['video_url'] ?? fm['link'];
          }
        }
        ...
        if (rawProducts is List) {
          for (final p in rawProducts) {
            ...
            final name = (pm['product_name'] ?? pm['name'] ?? pm['title'] ?? '').toString().trim();
            final desc = (pm['product_description'] ?? pm['description'] ?? '').toString().trim();
            final img = (pm['promote_img'] ?? pm['image'] ?? pm['img'] ?? pm['imageUrl'] ?? '').toString().trim();
            final link = (pm['visit_link'] ?? pm['website_url'] ?? pm['websiteUrl'] ?? pm['url'] ?? pm['link'] ?? '').toString().trim();
        ...
        return {
          ...
          'tags': tags,
          'brandName': (item['brandName'] ?? item['ad_company_name'] ?? item['company_name'] ?? '').toString(),
          'rating': 4.0,
          'products': products,
        };
      }).toList();
